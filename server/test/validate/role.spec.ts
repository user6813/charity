...
import { CreateRoleValidate, UpdateRoleValidate } from '../../src/validate/role';
...
describe('Permission & Role Validation Middleware', () => {
    ...
    test('should validate and proceed for CreateRoleValidate', async () => {
        const mockResult = { success: true, data: {} };
        (ValidateInput as jest.Mock).mockResolvedValue(mockResult);
        
        await CreateRoleValidate(req as Request, res as Response, next);
        
        expect(ValidateInput).toHaveBeenCalledWith(RoleDto, req.body);
        expect(req.body).toEqual(mockResult.data);
        expect(next).toHaveBeenCalled();
    });

    test('should return BAD_REQUEST for invalid CreateRoleValidate input', async () => {
        const mockResult = { success: false };
        (ValidateInput as jest.Mock).mockResolvedValue(mockResult);
    ...
    test('should validate and proceed for UpdateRoleValidate', async () => {
        const mockResult = { success: true, data: {} };
        (ValidateInput as jest.Mock).mockResolvedValue(mockResult);
        
        await UpdateRoleValidate(req as Request, res as Response, next);
        
        expect(ValidateInput).toHaveBeenCalledWith(UpdateRoleDto, req.body);
        expect(req.body).toEqual(mockResult.data);
        expect(next).toHaveBeenCalled();
    });

    test('should return BAD_REQUEST for invalid UpdateRoleValidate input', async () => {
        const mockResult = { success: false };
        (ValidateInput as jest.Mock).mockResolvedValue(mockResult);
